...
import com.gumamobile.mapbystep.domain.rememberHealthPermissionLauncher
import com.gumamobile.mapbystep.theme.spacing
import com.gumamobile.mapbystep.ui.common.Card
import com.gumamobile.mapbystep.ui.common.ErrorCard
...
fun ProfileScreen(
    ...
    val launcher = rememberHealthPermissionLauncher(
        ...
        onResult = {
            viewModel.onAction(ProfileAction.CheckPermissions)
        },
    )
    ...
    Column(
        ...
        AnimatedContent(targetState = state.healthPermissionState) { permissionState ->
            when (permissionState) {
                HealthPermissionState.Loading -> {}
                HealthPermissionState.Granted -> {}
                HealthPermissionState.NotGranted -> ErrorCard(
                    errorMessage = "Permissions not granted",
                    actionLabel = "Request permissions",
                    onAction = onPermissionRequest,
                )

                HealthPermissionState.NotInstalled -> ErrorCard(errorMessage = "Required Health Connect app not installed")
            }
        }
